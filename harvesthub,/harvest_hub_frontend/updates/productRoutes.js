...
// ================================
router.get("/", (req, res) => {
  const { farmer_id, location } = req.query; // optional location from frontend

  let sql = `
    SELECT 
      p.*, 
      u.fullName AS farmer_name, 
      u.location AS farm_location,
      p.created_at
    FROM products p
  ...
  // Optional filter if a farmer_id is given
  if (farmer_id) {
    sql += " WHERE p.farmer_id = ?";
    params.push(farmer_id);
  }

  // If location provided, prioritize sorting
  if (location) {
    sql += `
      ORDER BY 
        CASE 
          WHEN LOWER(u.location) LIKE LOWER(?) THEN 0 
          ELSE 1 
        END, 
        p.id DESC
    `;
